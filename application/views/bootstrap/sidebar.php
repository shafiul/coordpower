<div class="span2">
    <div class="well sidebar-nav">
        <ul class="nav nav-list">
            
            <li class="nav-header"><?php echo lang('menu_user_head');  ?></li>
            <li ><a href="#"><?php echo lang('menu_user_new');  ?></a></li>
            
            <li class="nav-header"><?php echo lang('menu_committee_head');  ?></li>
            <li ><a href="#"><?php echo lang('menu_committee_new');  ?></a></li>
            
            
            <li class="nav-header"><?php echo lang('menu_meeting_head');  ?></li>
            <li ><a href="#"><?php echo lang('menu_meeting_new');  ?></a></li>
            
            <li class="nav-header"><?php echo lang('menu_report_head');  ?></li>
            <li ><a href="#"><?php echo lang('menu_report_all');  ?></a></li>



        </ul>
    </div><!--/.well -->
</div><!--/span-->